#!/usr/bin/env python3
"""
Build an abstract 3D connectome layout from the eonsystemspbc/fly-brain repo.
...
Output:
  - JSON file consumable by the included Vision Pro / browser viewer.
...
not packaged neuron meshes for direct spatial rendering.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Export an abstract 3D fly-brain connectome layout for browser or "
            "Vision Pro viewing."
        )
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("flybrain_for_vision_pro.json"),
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Optional text file with one FlyWire root ID per line. These nodes are "
            "always included and highlighted."
        ),
    )
    parser.add_argument(
        "--seed",
...
def spectral_layout(
    ...
    try:
        ...
        # Give high-degree nodes a slightly larger radius to open up dense cores.
...
def color_gradient(t: float) -> list[float]:
    """Small blue→cyan→yellow gradient for degree coloring."""
...
def pick_nodes(
    ...
    focus_indices = {
        flywire_to_index[fid]
        for fid in focus_ids
        if fid in flywire_to_index
    }
...
def build_export(
    ...
    if subset.empty:
        raise ValueError(
            "No edges survived filtering. Try increasing --max-nodes or remove overly "
            "restrictive --focus-ids-file settings."
        )
    ...
    # Degree within the filtered edge set for sizing/coloring.
    ...
    return {
        "meta": {
            ...
            "notes": (
                ...
                "mesh. Use FlyWire meshes/skeletons if you need true morphology."
            ),
        },
        "nodes": nodes,
        "edges": edges,
...
def main() -> int:
    ...
    print(
        "[tip] For Vision Pro, start with ~2k-4k nodes and <=12k edges. "
        "Then open the included HTML viewer in Safari."
    )
    return 0
